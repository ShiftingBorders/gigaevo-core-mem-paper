...
from tools.evolution_data_utils import (
    RedisRunConfig,
...
def _parse_run_arg(arg: str, default_host: str, default_port: int) -> RedisRunConfig:
    ...
    return RedisRunConfig(
        ...
        redis_db=db,
        redis_prefix=prefix,
        label=label or f"{prefix}@{db}",
    )


async def _load_runs(configs: List[RedisRunConfig]):
    tasks = [fetch_evolution_dataframe(cfg) for cfg in configs]
            ...
            if annotate_frontier:
                ...
                if not fp.empty:
                    ...
                    if not improvements.empty:
                        ...
                        if frontier_annotations_max > 0 and len(improvements) > frontier_annotations_max:
                            idxs = np.linspace(0, len(improvements) - 1, frontier_annotations_max, dtype=int)
...
async def main():
    ...
    parser.add_argument("--redis-port", type=int, default=6379, help="Default Redis port")
    parser.add_argument(
        "--run",
        action="append",
        required=True,
    ...
    logger.info(f"Loaded {len(run_configs)} runs: {[c.display_label() for c in run_configs]}")
    ...
    for cfg, df in zip(run_configs, dfs):
        ...
        if prepared_df.empty:
            logger.warning(f"Run '{cfg.display_label()}': no valid iteration/fitness data after filtering")
            continue
