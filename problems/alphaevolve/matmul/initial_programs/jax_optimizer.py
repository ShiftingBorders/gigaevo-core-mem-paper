from helper import build_matmul_tensor
...
def get_matrix_multiplication_tensor(n, m, p):
    return jnp.array(build_matmul_tensor(n, m, p, dtype=np.float32))
...
def _run():
    ...
    return final_decomposition_np, float(final_loss), hypers.rank
...
def entrypoint():
    """
    Two-phase JAX optimization with straight-through rounding

    Returns:
        (decomposition, n, m, p, loss, rank) where decomposition = (U, V, W) factor matrices
    """
    decomposition, loss, rank = _run()
    return decomposition, n, m, p, float(loss), int(rank)