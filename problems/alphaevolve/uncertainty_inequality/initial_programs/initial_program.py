...
def _c_from_hermite_coeffs(hermite_coeffs: np.ndarray, num_hermite_coeffs: int):
...
def get_c_from_params(params: np.ndarray, hypers: Hyperparameters):
    c_others, log_c_last = params[:-1], params[-1]
    ...
    c, rmax = _c_from_hermite_coeffs(hermite_coeffs, hypers.num_hermite_coeffs)
    if c is None:
...
def entrypoint() -> np.ndarray:
    ...
    return np.array(best_coeffs[:hypers.num_hermite_coeffs])
