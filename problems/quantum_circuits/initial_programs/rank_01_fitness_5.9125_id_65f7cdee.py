...
def enhanced_bce_loss_with_adaptive_alignment(target: jnp.ndarray, factors: jnp.ndarray, temperature: float = 1.0, n: int = 1, r: int = 1) -> jnp.ndarray:
    ...
    binary_scale = jnp.minimum(2.0, 15.0 / (n * jnp.sqrt(r))) * (1.0 + 0.5 * tensor_density)
...
def adaptive_min_rank_search(
    ...
    adaptive_lr = lr * jnp.minimum(1.0, jnp.sqrt(10.0 / n)) * jnp.sqrt(jnp.maximum(density, 0.05))
