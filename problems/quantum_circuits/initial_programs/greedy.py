...
def _sample_rank1(key, d: int):
    ...
    v = jnp.where(v.sum() == 0, v.at[idx].set(1.0), v)
    return ks[0], v
...
def search_min_rank(
    T: jnp.ndarray, samples=128, max_rank=64, tol=1e-6, seed=0
) -> jnp.array:
    ...
    for _ in range(max_rank):
        ...
        if best is None or cur < tol:
            break
        R = R - reconstruct_from_single_binary_factor(best)
        decomposed.append(best)
        cur = sc
        if cur < tol:
            break
...
def entrypoint(context: List[Data]) -> List[jnp.array]:
    ...
    for i, data in enumerate(context):
        res.append(
            search_min_rank(
                T=data.tensor, **get_parametes_based_on_context_data(data, seed=i + 1)
            )
        )
    return res
