...
def make_trainer(target: jnp.ndarray, lr: float, sparsity: float):
    ...
    def loss_fn(f: jnp.ndarray, step: int) -> jnp.ndarray:
        ...
        current_temp = jnp.maximum(1.0 / jnp.sqrt(step + 1000.0), 0.05)  # Slower cooling than Parent 2
...
def warm_start_expansion(F_prev: jnp.ndarray, new_rank: int, base_key, seed: int) -> jnp.ndarray:
...
def progressive_greedy_backup(T: jnp.ndarray, max_rank: int, seed: int) -> jnp.array:
    ...
    adaptive_max_rank = max_rank + min(10, initial_residual // (n * 2))  # Dynamic expansion
...
def progressive_min_rank_search(
    ...
    initial_residual = get_residual_num(T)
    progressive_expansion = min(max_expansion + 3, max(5, n // 4 + int(4 * density + initial_residual // (n * 5))))
