...
def validate(output):
    ...
    n, d = output.shape
    if d != 12:
        raise ValueError(f"Each vector must have 12 dimensions (got shape={output.shape}).")
    ...
    for i, row in enumerate(output):
        if not np.any(row):
            raise ValueError(f"Zero vector found at index {i}. All vectors must be non-zero.")
    ...
    def sq_dist(a, b):
        return sum(int(ai - bi) * int(ai - bi) for ai, bi in zip(a.tolist(), b.tolist()))
    ...
    if len(set(r2_list)) != 1:
        raise ValueError(f"Single-shell violation: norms not all equal (found values {set(r2_list)}).")
    ...
    if d2_min is None:
        raise ValueError("A single vector (N=1) is insufficient. Need at least two vectors to define pairwise separation.")
