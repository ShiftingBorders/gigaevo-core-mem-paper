...
class MutationContext(BaseModel, ABC):
    ...
    def get_metadata(self) -> dict[str, Any]:
        """Get metadata about this context for logging."""
        pass
...
class InsightsMutationContext(MutationContext):
    ...
    def format(self) -> str:
        ...
        for insight in self.insights.insights:
            lines.append(f"- **[{insight.type}]**[{insight.tag}]**[{insight.severity}]**: {insight.insight}")
    ...
    def get_metadata(self) -> dict[str, Any]:
        return {
            ...
            "insight_count": len(self.insights.insights)
        }
...
class FamilyTreeMutationContext(MutationContext):
    ...
    ancestors: list[LineageAnalysis]
    descendants: list[LineageAnalysis]
    metrics_formatter: MetricsFormatter
    ...
    def format(self) -> str:
        ...
        logger.debug(f"[FamilyTreeMutationContext] Formatting with {len(self.ancestors)} ancestors and {len(self.descendants)} descendants")
        
        if self.ancestors:
            lines.append("### Ancestors")
            lines.append("")
            for i, analysis in enumerate(self.ancestors):
                lines.append(f"#### Ancestor {i+1}: {analysis.from_id[:8]} → {analysis.to_id[:8]}")
                lines.append("")
                lines.append(self._format_lineage_block(analysis))
                lines.append("")
        
        if self.descendants:
            logger.debug(f"[FamilyTreeMutationContext] Adding descendants section with {len(self.descendants)} descendants")
            lines.append("### Descendants")
            lines.append("")
            for i, analysis in enumerate(self.descendants):
                logger.debug(f"[FamilyTreeMutationContext] Adding descendant {i+1}: {analysis.from_id[:8]} → {analysis.to_id[:8]}")
                lines.append(f"#### Descendant {i+1}: {analysis.from_id[:8]} → {analysis.to_id[:8]}")
                lines.append("")
                lines.append(self._format_lineage_block(analysis))
                lines.append("")
        else:
        ...
        logger.debug(f"[FamilyTreeMutationContext] Formatted result length: {len(result)}")
        logger.debug(f"[FamilyTreeMutationContext] Formatted result contains '### Descendants': {'### Descendants' in result}")
        
        return result
    
    def _format_lineage_block(self, analysis: LineageAnalysis) -> str:
...
class CompositeMutationContext(MutationContext):
    ...
    def get_metadata(self) -> dict[str, Any]:
        metadata: dict[str, Any] = {
            ...
            "component_count": len(self.contexts)
        }
