...
class InsightsAgent(LangGraphAgent):
    ...
    def build_prompt(self, state: InsightsState) -> InsightsState:
        ...
        # Build error section (agent responsibility!)
        errors = program.get_all_errors_summary()
        error_section = f"\n\\n- **Error Analysis**: Focus on fixing or avoiding failure modes from stages:\n{errors}" if errors else ""
        
        # Format prompts (agent responsibility!)
