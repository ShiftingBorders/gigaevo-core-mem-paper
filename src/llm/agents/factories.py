...
def create_insights_agent(
    ...
    system_prompt = system_template.format(task_description=task_description, max_insights=max_insights, metrics_description=metrics_formatter.format_metrics_description())
...
def create_scoring_agent(
    ...
    Example:
        >>> agent = create_scoring_agent(llm=llm)
        >>> score = await agent.arun(
        ...     program=program,
