"""Mutation agent for evolutionary code generation.

This agent encapsulates the logic for generating mutated programs using LLMs.
It supports both full rewrite and diff-based mutation modes.
"""
...
class MutationPromptFields(BaseModel):
    """Type-safe fields for mutation prompt templates.
    
    This model defines all fields that can be used in mutation prompt templates,
    ...
    parent_blocks: str = Field(description="Formatted parent program blocks with code, metrics, insights")
...
class MutationAgent(LangGraphAgent):
    ...
    async def arun(self, input: list[Program], metadata: dict | None = None) -> dict:
        """Execute mutation agent.
        
        Args:
            input: List of parent programs to mutate
            metadata: Optional metadata (e.g., mutation_mode override)
        ...
        initial_state: MutationState = {
            "input": input,
            "mutation_mode": metadata.get("mutation_mode", self.mutation_mode) if metadata else self.mutation_mode,
            "messages": [],
            "llm_response": None,
            "final_code": "",
            "mutation_label": "",
    ...
    def build_prompt(self, state: MutationState) -> MutationState:
        ...
        for i, p in enumerate(parents):
            ...
            formatted_context = p.metadata.get("mutation_context", "")
